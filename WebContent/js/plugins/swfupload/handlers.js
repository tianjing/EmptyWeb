...
function swfUploadPreLoad() {
	...
	var loading = function () {
		...
		//ZJ,2015-05-04：增加判断标签是否为空
		if(document.getElementById("divLoadingContent") != null){
...
function swfUploadLoaded() {
	...
	if(document.getElementById("divLongLoading") != null ){
		document.getElementById("divLongLoading").style.display = "none";
	}
	if(document.getElementById("divAlternateContent") != null){
...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			alert("您试图上传的文件太多.\n" + (message === 0 ? "您已达到上传限制." : "您最多只能选择" + (message > 1 ? "上传 " + message + "个文件." : "1个文件.")));
			return;
		}
